...
class ClarisseEngine(Engine):
    ...
    def init_engine(self):
        ...
        if current_os not in ["darwin", "win32", "linux"]: # use linux2 in Python2
            raise sgtk.TankError(
                "The current platform is not supported! Supported platforms "
                "are Mac, Linux and Windows64."
            )
...
def show_error(msg):
    ...
    ix.application.message_box(
        msg,
        str("Shotgun Error | Clarisse engine"),
...
def show_warning(msg):
    ix.application.message_box(
        msg,
        str("Shotgun Warning | Clarisse engine"),
        ix.api.AppDialog.cancel(),
        ix.api.AppDialog.STYLE_OK,
    )
...
def show_info(msg):
    ix.application.message_box(
        msg,
        str("Shotgun Info | Clarisse engine"),
        ix.api.AppDialog.cancel(),
        ix.api.AppDialog.STYLE_OK,
    )
